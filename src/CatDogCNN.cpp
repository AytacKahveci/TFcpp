...
Status CatDogCNN::Save()
{
    tensorflow::GraphDef graph_def;
    TF_CHECK_OK(t_root.ToGraphDef(&graph_def));
    //LOG(INFO) << __FUNCTION__ << "----------------------------frozen_graph_def.ShortDebugString():" << std::endl << graph_def.ShortDebugString();

    const std::string filename = "/media/aytac/Tank/machine-learning/TFcpp/graphs/cat_dog.pb";
...
Status CatDogCNN::LoadSavedModel(string& file_name)
{
    ...
    TF_CHECK_OK(ReadBinaryProto(Env::Default(), file_name, &graph_def));

    Status status =  f_session->Create(graph_def);
...
Status CatDogCNN::PredictFromFrozen(Tensor& image, int& result)
{
    ...
    TF_CHECK_OK(f_session->Run({{"Conv1_layer_input", image}}, {"fcfinal/Softmax"}, {}, &out_tensors));
